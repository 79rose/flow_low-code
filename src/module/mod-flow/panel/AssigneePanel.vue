<script setup lang="ts">
  import type { AssigneeNode, Field } from '../node/type'
...
<template>
  <el-form label-position="top" label-width="90px">
    ...
    <el-form-item prop="roles" label="指定角色" v-if="activeData.assigneeType === 'role'">
      <RoleSelector v-model="activeData.roles" style="width: 220px" collapse-tags :max-collapse-tags="1" multiple
    ...
    <el-form-item prop="formUser" label="表单内人员" v-if="activeData.assigneeType === 'formUser'">
      <el-select placeholder="选择表单内人员" style="width: 220px" v-model="activeData.formUser">
        <el-option v-for="item in fields.filter((e) => e.name === 'UserSelector')" :key="item.id" :label="item.label"
          :value="item.id" />
      </el-select>
    </el-form-item>
    <el-form-item prop="formRole" label="表单内角色" v-if="activeData.assigneeType === 'formRole'">
      <el-select placeholder="选择表单内角色" style="width: 220px" v-model="activeData.formRole">
        <el-option v-for="item in fields.filter((e) => e.name === 'RoleSelector')" :key="item.id" :label="item.label"
          :value="item.id" />
      </el-select>
    </el-form-item>
  </el-form>
