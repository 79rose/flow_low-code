<script setup lang="ts" name="AdvancedFilter">
  ...
  import type { Field } from 'module/mod-flow/node/type'
...
<template>
  <div class="filter-container">
    <div class="logical-operator">
      ...
      <el-switch v-model="filterRules.operator" inline-prompt
        style="--el-switch-on-color: #409eff; --el-switch-off-color: #67c23a" active-value="and" inactive-value="or"
    ...
    <div class="filter-option-content">
      <el-form :label-width="0" :inline="true" :model="filterRules">
        <el-row v-for="(item, index) in filterRules.conditions" :key="`${item.field}-${index}`" :gutter="5"
          ...
          <el-col :xs="24" :sm="7">
            <el-form-item :prop="'conditions.' + index + '.field'" style="width: 100%">
              <trigger ref="triggerRef" :options="$props.filterFields.filter((e) => e.value !== undefined)"
                :filter-rules="filterRules" v-model="item.field" @update:model-value="item.value = null" />
            </el-form-item>
          </el-col>
          ...
          <el-col :xs="24" :sm="10" v-if="item.field">
            <el-form-item :prop="'conditions.' + index + '.value'" style="width: 100%">
              <Render :field="$props.filterFields.find((e) => e.id === item.field) as Field" v-model="item.value" />
            </el-form-item>
          </el-col>
        ...
        <AdvancedFilter v-for="(_item, index) in filterRules.groups" :key="index" @delGroup="delGroup(index)"
          v-model="filterRules.groups[index]" :filterFields="filterFields">
